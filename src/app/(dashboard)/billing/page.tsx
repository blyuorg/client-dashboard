...
export default async function BillingPage() {
  ...
  const [{ data: invoices }, { data: projects }, { data: profile }, { data: payments }] = await Promise.all([
    ...
    supabase
      .from("payments")
      .select("id, invoice_id, amount, method, status, paid_at, transaction_id")
      .eq("client_id", user!.id)
      .order("created_at", { ascending: false }),
  ]);
  const projectTitles = new Map((projects ?? []).map(p => [p.id, p.title]));
  const invoiceNumbers = new Map((invoices ?? []).map(i => [i.id, i.invoice_number]));
  ...
  const paymentRows = (payments ?? []).map(p => ({
    id: p.id,
    invoiceNumber: invoiceNumbers.get(p.invoice_id) ?? "—",
